...
-(void)setUpWriter
{
    ...
    //新改的压缩参数2018.03.14  6.0 7.0 3.0 合适6.0 30 30
    videoCompression = @{ AVVideoAverageBitRateKey : @(pixelNumber * 6.0),//10.1相当于AVCaptureSessionPresetHigh数字越大越精细
            ...
            [_videoWriter finishWritingWithCompletionHandler:^{
                ...
                void (^completion)(void) = ^() {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (completionBlock) completionBlock([[_videoWriter outputURL] path]);
                        [self cleanup];
                    });
                };
                completion();
    ...
    dispatch_async(_render_queue, ^{
        ...
        dispatch_sync(dispatch_get_main_queue(), ^{
            UIGraphicsPushContext(bitmapContext); {
                ...
                if (self.recordView) {
                    [self.recordView drawViewHierarchyInRect:CGRectMake(0, 0, _viewSize.width, _viewSize.height) afterScreenUpdates:NO];
                }
                //}
